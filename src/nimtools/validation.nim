...
template requireNonEmpty*[T](s: seq[T], operation: string = "operation"): seq[T] =
...
template requirePositive*(n: int, name: string = "value"): int =
...
template requireNonZero*(n: SomeNumber, name: string = "value"): auto =
...
template safeFirst*[T](s: seq[T]): T =
  ...
  let validated = s.requireNonEmpty("first")
  validated[0]
...
template safeLast*[T](s: seq[T]): T =
  ...
  let validated = s.requireNonEmpty("last")
  validated[^1]
...
template safeReduce*[T](s: seq[T], operation: proc(a, b: T): T): T =
  ...
  let validated = s.requireNonEmpty("reduce")
  var result = validated[0]
  for i in 1..<validated.len:
    result = operation(result, validated[i])
  result
...
template safeChunk*[T](s: seq[T], size: int): seq[seq[T]] =
  ...
  let validatedSize = size.requirePositive("chunk size")
  var result: seq[seq[T]] = @[]
  var i = 0
  while i < s.len:
    let endIdx = min(i + validatedSize, s.len)
...
template safeDivisibleBy*(n: SomeInteger, divisor: SomeInteger): bool =
  ...
  let validatedDivisor = divisor.requireNonZero("divisor")
  (n mod validatedDivisor) == 0
